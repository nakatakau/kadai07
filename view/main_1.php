...
session_start();
// 検索数
if (isset($_GET['color_select_select'])) {
  $_SESSION['color']  = $_GET['color_select_select'];
}
if (isset($_GET['search_num_select'])) {
  $_SESSION['search'] = $_GET['search_num_select'];
} else {
  $_SESSION['search'] = 20;
}
if (isset($_GET['page'])) {
...
$def = (int) $_SESSION['search'] * (int) $_SESSION['page'];
$pre_def = (int) $_SESSION['search'] * ((int) $_SESSION['page'] - 1);
...
<body>
  ...
  // ------------------------------------------------
  // アイテムリストの呼び出し
  if ($_SESSION['cate'] != "" && $_SESSION['color'] != "") {
    $stmt = $pdo->prepare("SELECT * FROM itemlist WHERE small_category = :cate, color = :color ORDER BY itemcode DESC");
    $stmt->bindValue(':cate', $_SESSION['cate'], PDO::PARAM_STR);
    $stmt->bindValue(':color', $_SESSION['color'], PDO::PARAM_STR);
  } elseif ($_SESSION['cate'] == "" && $_SESSION['color'] != "") {
    $stmt = $pdo->prepare("SELECT * FROM itemlist WHERE color = :color ORDER BY itemcode DESC");
    $stmt->bindValue(':color', $_SESSION['color'], PDO::PARAM_STR);
  } elseif ($_SESSION['cate'] != "" && $_SESSION['color'] == "") {
    $stmt = $pdo->prepare("SELECT * FROM itemlist WHERE small_category = :cate ORDER BY itemcode DESC");
    $stmt->bindValue(':cate', $_SESSION['cate'], PDO::PARAM_STR);
  } else {
  ...
  <script>
    ...
    let num = <?= $def ?>;
    let pre_page = <?= $pre_def ?>;
    let ck = <?= $search ?>;
    if (data.length - pre_page < ck) {
      num = data.length;
    }
    for (let i = pre_page; i < num; i++) {
      // ------------------------------------------
      // divの作成
      // ------------------------------------------
    ...
    let list_number = <?= $search ?>;
    const page_list = Math.ceil(Number(data.length) / Number(list_number));
    let count = <?= $page ?>;
    const start = <?= $page ?>;
    if (start <= 3 && start >= 2 && page_list >= 2) {
      if (start == 2) {
        const page = document.createElement('li');
        page.textContent = start - 1;
        page_count.appendChild(page);
      } else {
        const page2 = document.createElement('li');
        const page1 = document.createElement('li');
        page2.textContent = start - 1;
        page1.textContent = start - 2;
        page_count.appendChild(page1);
        page_count.appendChild(page2);
      }
    }
    //最終ページから最後の３ページを出す
    if (start >= page_list - 1 && page_list >= 4) {
      const page = document.createElement('li');
      page.textContent = start - 1;
      page_count.appendChild(page);
    }
    for (count; count <= page_list; count++) {
      // 基準ページから+2と最終ページを追加
      if (!(count > start + 2 && count < page_list)) {
        const page = document.createElement('li');
    ...
    if (count > 4 && page_list > 4) {
      if (start >= 4) {
        const page = document.createElement('li');
        page.textContent = 1;
        const first_page = page_count.children[0];
        first_page.before(page);
        const dot1 = document.createElement('p');
        dot1.style.pointerEvents = "none";
        dot1.textContent = "‥"
        const first_next = page_count.firstElementChild;
        first_next.after(dot1);
      }
      if (start < page_list - 3) {
        const dot2 = document.createElement('p');
        dot2.style.pointerEvents = "none";
        dot2.textContent = "‥"
        const last_second = page_count.lastElementChild;
        last_second.before(dot2);
      }
    }
    // ---------------------------------------------
    // formの操作
    ...
    // セレクトの指定
    const search = <?= $_SESSION['search'] ?>;
    search_num_select.childNodes.forEach(target => {
